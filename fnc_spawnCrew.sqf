/*
	...
	_this select 4: (optional) crew type (Array)
...
if ((typeName _crewType)!= "ARRAY") exitWith {debugLog "Log: [spawnCrew] Crew type (4) must be a Array!"; []};
...
if ((_hasDriver == 1) && (isNull (driver _vehicle))) then
{
	if (!_dummy) then
	{
		_unit = _grp createUnit [_crewType call BIS_fnc_selectRandom, position _vehicle, [], 0, "NONE"];
		_crew = _crew + [_unit];
